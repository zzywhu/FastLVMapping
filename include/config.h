...
public:
    ...
    const CameraParams& cameraParams() const { return camera_params_; }
    const CalibParams& calibParams() const { return calib_params_; }
    const PointCloudParams& pointCloudParams() const { return pc_params_; }
    const ProjectionParams& projectionParams() const { return proj_params_; }
    const PnPParams& pnpParams() const { return pnp_params_; }
    const OutputParams& outputParams() const { return output_params_; }
    const ProcessingParams& processingParams() const { return processing_params_; }
    
    // Mutable getters for parameter groups
    CameraParams& mutableCameraParams() { return camera_params_; }
    CalibParams& mutableCalibParams() { return calib_params_; }
    PointCloudParams& mutablePointCloudParams() { return pc_params_; }
    ProjectionParams& mutableProjectionParams() { return proj_params_; }
    PnPParams& mutablePnpParams() { return pnp_params_; }
    OutputParams& mutableOutputParams() { return output_params_; }
    ProcessingParams& mutableProcessingParams() { return processing_params_; }
